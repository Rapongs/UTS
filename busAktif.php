<?php 
    ...
    $bus = showData("SELECT * FROM bus WHERE status = 1");
?>
...
<body>
    ...
    <table class="table table-bordered border-black" style="margin-top: 30px;">
        ...
        <tr class="row<?php if($dataBus["status"]==1){
            echo "1";
        }elseif($dataBus["status"]==2){
            echo "2";
        }else{
            echo "3";
        } ?>">
            <td><?= $dataBus["id_bus"];  ?></td>
            <td style="padding: 10px;"><a style="color: black;" href="updateBus.php">Update</a> | <a
                    style="color: black;" href="hapusBus.php">Delete</a></td>
            <td><?= $dataBus["plat"];  ?></td>
            <td><?= $dataBus["status"];  ?></td>
            <td>100</td>
        </tr>
        <?php endforeach; ?>
    </table>
